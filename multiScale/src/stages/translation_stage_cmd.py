...
class SLC_translationstage:
    ...
    def __init__(self, locator):
        ...
        :param locator: Stage ID/address of the rotation stage, e.g. 'usb:sn:MCS2-00001723'
        """
    ...
    def assert_lib_compatibility(self):
        ...
        incompatibilities. Raises a RuntimeError if the major version numbers are different.
        """
    ...
    def findReference(self):
        """
        Find the reference position for all three channels (stage axes). Move the up-down stage (channel 2) first to avoid collisions
        with the illumination and detection objectives. Set the positions to zero after referencing.
        """
        ...
        for channel in channel_array:
            ...
            self._waitForEvent_referencing()
    ...
    def _waitForEvent_referencing(self):
        """
        Wait for the referencing operation of one stage to be completed. Called from findReference().
        """
        ...
        try:
            ...
            if event.type == ctl.EventType.REFERENCE_FOUND:
                if (event.i32 == ctl.ErrorCode.NONE):
                    ...
                    self._waitForEvent_MovementComplete()
                else:
    ...
    def _waitForEvent_MovementComplete_Channels(self, nb_channels):
        """
        Wait for until movement_finished flag is sent by all channels/stages.

        :param nb_channels: number of stages to send a movement_finished signal.
        """
        iter =0
        timeout = 30000  # in ms
        done = False
    ...
    def _waitForEvent_MovementComplete(self):
        """
        Wait for movement complete events generated by the connected device. Called from find_reference() after reference has been found - wait for stage to move.
    ...
    def _waitForEvent_stream(self):
        """
        Wait for events generated by the connected device. Wait for stream to finish in stack acquisitions.
        """
        while True:
            try:
    ...
    def _waitForEvent_CommandGroupTriggered(self):
        """
        Wait for events generated by the connected device if several commands generated and triggered.
        """
    ...
    #         event_handle_thread = Thread(target=self._waitForEvent_stream)
    ...
    def moveToPosition(self, position_list):
        ...
        :param position_list: np.array with 3 values for 3 channels/axes.
        """
        ...
        self._waitForEvent_CommandGroupTriggered()
        ...
        self._waitForEvent_MovementComplete_Channels(3)
    ...
    def streamStackAcquisition_externalTrigger_setup(self, no_of_frames, increment, slow_velocity, slow_acceleration):
        """
        Set up a stack acquisition by preparing the stages to receive an external trigger signal that will move the stage by increment.
        no_of_frames trigger signals are expected. A waiting stream (thread) to receive voltage inputs is started.
        ...
        try:
            # Spawn a thread to receive events from the controller.
            self.event_handle_thread_triggeredStream = Thread(target=self._waitForEvent_stream)
